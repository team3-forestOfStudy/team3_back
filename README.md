...
```
📁src
├── controllers    # 요청/응답 처리 (HTTP 관점)
├── services       # 비즈니스 로직 (도메인 관점)
├── routes         # 라우터 정의 (URL → Controller)
├── prisma         # Prisma Client 및 스키마
├── openapi.yaml   # Swagger 문서
├── app.js         # Express App 생성 및 미들웨어 설정
└── server.js      # 서버 실행부 (포트 listen)
```
...
| **스터디 · 집중 · 이모지 도메인** | 이수진 | 스터디 CRUD, 비밀번호 확인, 홈 목록(검색/정렬/페이지네이션), 오늘의 집중, 포인트, 이모지 API 구현 |
